...
1. BTC turbulence (recent_vol / long_vol > 1.5x) — 알트 진입 차단
2. 알트 동시 포지션 ≤ 3
3. ETH 1h trend (close < SMA20 × 0.98) — 알트 차단
4. **BTC 멀티 TF 합의** (5m/15m/1h/4h/1d 중 ≥3개 TF에서 close < SMA20) — 알트 차단
...
- **custom_stake_amount**: experience_log 통계로 quarter-Kelly 계산 + fusion_prob confidence 곱. 30건 미만이면 fallback heuristic. cap 20%
- **adjust_trade_position**: -3% / -6% / -9% 진입 시 추가 0.5x씩 (최대 3회). 단, **fusion_prob ≥ buy threshold** + HMM != bear일 때만 (악화된 thesis에는 절대 추가 매수 안 함)
...
- `tests/` — 137건 단위테스트 (fusion 51 + experience 11 + validation 20 + orderbook 18 + sizing 19 + configs 11 + status 6 + HMM 1)
...
## 거래 대상 (라이브)
**Dynamic PairList**로 Upbit KRW 전체에서 자동 선정:
- VolumePairList (top 30, min 50억 KRW/일) → AgeFilter (30일+) → PriceFilter → SpreadFilter (0.5%↓) → RangeStability → VolatilityFilter → PrecisionFilter → VolumePairList 재정렬 top 15
- blacklist: 스테이블코인 (USDT/USDC/DAI/BUSD)
- BTC/KRW는 거래량 1위라 항상 포함. informative_pair로도 영구 구독.

## 거래 대상 (백테스트)
재현성을 위해 `config-backtest.json`에서 StaticPairList로 10종 핀:
