...
### v1 → v2 진화 이유
v1 verify 결과 (2026-06-08 14:43 KST): cold-start이 *진입 임계*는 풀었으나
confirm_trade_entry의 *시장 사실 가드*(BTC turbulence + ETH 1h trend + BTC multi-TF)가
3건씩 차단 → 실제 진입 0건. **v2: 시장 사실 가드도 완화 + stake 더 축소**.

| 파라미터 | 평시 | **Cold-start (exp<50)** |
|---|---|---|
| fusion_buy 임계 | 0.45 | **0.40** |
| fusion_buy의 Stage 2 SMA50 | 활성 | **제거** |
| Kelly cap | 15% | **1%** (페어당 ~₩10K, v1의 3%에서 추가 축소) |
| BTC turbulence multiplier | 1.5× | **2.5×** (큰 변동만 차단) |
| ETH 1h SMA20 tolerance | × 0.98 | **× 0.95** (5% 하락까지 허용) |
| BTC multi-TF block threshold | 5/5 | **7/5 (실질 비활성)** |
| 시스템 손실 한도 (-1% 변동) | ~₩9.5K | **~₩500** |
| fusion_strong, rsi_bounce | 그대로 | 그대로 (고확신 진입은 동일) |
