...
5. **Orderbook 게이트** (Phase B): top-5 cumulative bid/ask imbalance > **-0.70** (Upbit KRW 시장 실측 기반 보정), spread ≤ 0.5%. microprice는 5레벨 inverse-distance weighted

## 진입 경로 (Multi-path, NFIX 참고)
- **fusion_strong**: fusion_prob ≥ 0.62 + do_predict=1 + vol > 20MA (고확신) — cold-start 무관
- **fusion_buy**: fusion_prob ≥ **0.45** (cold-start 시 **0.40**) + do_predict=1 + vol > 20MA (표준)
- **quick_scalp** (2026-06-08 신규): EMA9>EMA21 + close>VWAP_1h + RSI 30~55 + vol_spike (1.2× 20MA) — **단타 전용**. BTC 가드 면제, cold-start 무관 영구. 별도 ROI/SL/stake 정책.
...
- **Stage 2 필터**: fusion_strong/ta_breakout에 전체 SEPA (close > SMA50 > SMA150 > SMA200) 적용. **fusion_buy**는 `close > SMA50`만 (횡보장 대응 완화, **cold-start 시 제거**). rsi_bounce(평균회귀)는 면제

## 🔥 Cold-start 학습 가속 모드 (2026-06-08, v2)
**Experience < 50건일 때 자동 활성화** — chicken-egg(거래 없음→학습 못함→봇이 진입 안함) 깨기.

### v1 → v2 → v3 진화 이유
- **v1 verify** (2026-06-08 14:43 KST): 진입 임계 풀었으나 confirm_trade_entry의
  시장 사실 가드(BTC turbulence + ETH 1h + BTC multi-TF) 3건씩 차단 → 진입 0건
- **v2 verify** (15:50 KST): 시장 사실 가드 풀렸음 (BLOCKS 0건) 그러나 여전히
  진입 0건 → populate_entry_trend에서 enter_long=1 결정조차 못 함
  → 잔여 차단은 *populate_entry_trend의 vol_above_ma + do_predict* 확정
- **v3** (KST 16:40): vol_above_ma + do_predict도 fusion_buy 경로에서 면제
  → 마지막 안전망 (Kelly cap 1%, Initial SL, Orderbook 가드)만 유지

| 파라미터 | 평시 | **Cold-start (exp<50)** |
|---|---|---|
| fusion_buy 임계 | 0.45 | **0.40** |
| fusion_buy의 Stage 2 SMA50 | 활성 | **제거** |
| **fusion_buy의 vol_above_ma** (v3) | 활성 | **제거** |
| **fusion_buy의 do_predict** (v3) | 활성 | **제거** |
| Kelly cap | 15% | **1%** (페어당 ~₩10K) |
| BTC turbulence multiplier | 1.5× | **2.5×** |
| ETH 1h SMA20 tolerance | × 0.98 | **× 0.95** |
| BTC multi-TF block threshold | 5/5 | **7/5 (실질 비활성)** |
| 시스템 손실 한도 (-1% 변동) | ~₩9.5K | **~₩500** |
| fusion_strong, ta_breakout, rsi_bounce | 그대로 | 그대로 (고확신 진입은 동일) |

### 유지된 안전망
- ATR×1.2 initial SL (페어당 -1.5~-2%)
- Volume 가드 (vol > 20MA) 유지
- do_predict (DI threshold 0.9) 유지
- Orderbook 가드 (cum_imb > -0.70, spread ≤ 0.5%) 유지
- Alt position limit ≤ 3 유지
- dry_run이라 실손 0

### 자동화
50건 도달 시 평시 모드 자동 복귀. heartbeat 1분/회 `_cold_start_active` 갱신.
상태는 `strategy_state.json.cold_start.{active, effective_*}`에 노출.

**목표**: 1~2주 내 거래 50건 누적 → adaptive learning 본격 활성화.

## 프로텍션 (손실 연쇄 방어)
- **CooldownPeriod**: 거래 종료 후 2캔들(10분) 쿨다운 — 스톱 후 즉시 재진입 방지
- **StoplossGuard (per-pair)**: 1시간 내 같은 페어 스톱 2회 → 30분 잠금
- **StoplossGuard (global)**: 2시간 내 전체 스톱 4회 → 1시간 전체 거래 중지

## Quick Scalp 단타 경로 (2026-06-08 신설)
**철학**: 봇이 *장기 추세 follower*로 설계되어 단타 부적합 → 별도 진입 경로 추가.

### 진입 조건 (모두 만족)
- `ema_9 > ema_21` (5m 단기 momentum 상승)
- `close > vwap_1h` (intraday 강세)
- `RSI 14: 30~55` (oversold 반등 시작 영역)
- `vol_spike_scalp`: 거래량 > 20MA × 1.2

### 청산 (별도 ROI ladder)
| 보유 시간 | 익절 조건 | exit_reason |
|---|---|---|
| 즉시 | +1.0% | `scalp_roi_1pct` |
| 5분+ | +0.5% | `scalp_roi_0.5pct` |
| 15분+ | +0.3% | `scalp_roi_0.3pct` |
| 30분+ | 0% (breakeven) | `scalp_roi_breakeven` |

### 별도 SL
- **-0.8% tight** (ATR trailing 없음, 단순 즉시 손절)

### 별도 stake
- **0.5% per trade** (bankroll 기반, 페어당 ~₩5K)
- Kelly / SQN / Bear scaling 모두 무관

### 면제된 가드
- BTC turbulence
- ETH 1h trend
- BTC multi-TF bearish block
- (단타라 cross-pair 영향 무관)

### 유지된 안전망
- Alt position limit ≤ 3
- Orderbook 가드 (cum_imb > -0.70, spread ≤ 0.5%)
- Cooldown 2캔들
- StoplossGuard (1h 2회 / 2h 4회)

## 청산 로직 (Quick-Swing v2)
...
- **custom_exit (Quick-profit)**: fusion이 진입 시점 대비 0.08 이상 하락 + 수익 0.3%+ → `exit_quick_fusion_fade`. 30캔들(150분) 보유 + 수익 0.2%+ → `exit_stale_profit`
- RSI 과매수 단독으로는 청산하지 않음 — fusion이 여전히 강할 때 winner를 조기 청산하지 않기 위함
- **custom_stoploss**: **진입 경로별 분리** — 모멘텀(fusion/ta_breakout): ATR×1.2 초기 + 4단계 trailing (**+2.5% ATR×0.5, +1.5% ATR×0.6, +0.8% ATR×0.7**, ATR활성 ATR×0.7), **breakeven +0.8%** (2026-05-28 winner 키우기 위해 완화). **rsi_bounce**: ATR×**2.0** 초기, breakeven +1.0%
- **ROI**: {"0": **2.0%**, "5": 1%, "10": 0.7%, "15": 0.5%, "30": 0.4%, "60": 0.3%}, **SL**: -1.5%
...
## Upbit API 최적화
- **Time-in-force**: `entry = "GTC"`, `exit = "GTC"` — Freqtrade가 Upbit에 대해 커스텀 TIF(PO 등)를 미지원
...
- 진입 주문은 `limit` (GTC), 긴급 청산만 `market`. Upbit은 PO(Post-Only) TIF 미지원
- `.env` 파일은 절대 커밋 금지
