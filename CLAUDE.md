...
**Experience < 50건일 때 자동 활성화** — chicken-egg(거래 없음→학습 못함→봇이 진입 안함) 깨기 위한 한시 모드:

| 파라미터 | 평시 | **Cold-start (exp<50)** |
|---|---|---|
| fusion_buy 임계 | 0.45 | **0.40** |
| fusion_buy의 Stage 2 SMA50 | 활성 | **제거** |
| Kelly cap | 15% | **3%** (페어당 ~₩30K) |
| 시스템 손실 한도 | 큼 | **-₩1.5K (-1% 변동 기준)** |
| fusion_strong, ta_breakout, rsi_bounce | 그대로 | 그대로 (고확신 진입은 동일) |

50건 도달 시 자동 평시 복귀. heartbeat 1분/회마다 `_cold_start_active` 갱신. 상태는 `strategy_state.json.cold_start`에 노출되어 대시보드에서 확인 가능.

**목표**: 1~2주 내 거래 50건 누적 → adaptive learning 본격 학습 데이터 확보.
