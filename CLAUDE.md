...
- **rsi_bounce**: 레짐별 분리 — bull/sideways: RSI<25+ta>10, **bear: RSI<30+ta>0** (haguri-peng 패턴) + close > SMA200×0.95. BTC 가드 면제
...
- **custom_stoploss**: ATR 기반 3단계 trailing (스캘핑 최적화) — +2% 이상 ATR×0.5 타이트 트레일, +1% 이상 ATR×0.6, 기본 ATR×1.5 활성화 후 ATR×0.8 트레일. +0.8% 이상에서 breakeven 보장
...
- **custom_stake_amount**: experience_log 통계로 quarter-Kelly 계산 + fusion_prob confidence 곱. 30건 미만이면 fallback heuristic. cap **15%** (페어당 fair share 19% 이하로 보수적). **SQN < 2.0이면 sizing 자동 50% 축소** (Van Tharp 시스템 품질 게이트)
